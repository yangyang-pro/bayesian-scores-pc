...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='CNetBIC Experiments')
    parser.add_argument('--alpha', type=float, default=0.01, help='Laplace smoothing factor')
    ...
    parser.add_argument('--output', type=str, default='result/cnet-bic', help='Output store path')
    ...
    csv_file = open(os.path.join(time_dir, 'cnet-bic.csv'), 'w')
    fieldnames = ['Dataset',
                  'train_ll', 'test_ll',
    ...
    for dataset in BINARY_DATASETS:
        ...
        cnet = CNet()
        start = time.time()
        cnet.bounded_search_bic(data=train, n_search_ors=n_search_ors, alpha=alpha)
        ...
        with open(os.path.join(model_dir, dataset + '.cnet.bic'), 'wb') as cnet_file:
